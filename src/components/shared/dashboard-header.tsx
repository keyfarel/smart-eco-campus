...
import { LogOut, User as UserIcon, Cloud, ChevronRight, Home, MapPin } from "lucide-react"
...
export function DashboardHeader() {
  ...
  // Dynamic role path for nested routing (Rule 11)
  const userRole = session?.user?.role
  const rolePrefix = userRole === "SUPER_ADMIN" ? "/super-admin" : userRole === "ADMIN_GEDUNG" ? "/admin-gedung" : ""
  const accountLink = `${rolePrefix}/account`
  
  // Resolve building location
  const assignedGedungId = (session?.user as any)?.assignedGedung
      ...
      {session?.user && (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-56 bg-zinc-950 border-zinc-800 text-zinc-300">
            <DropdownMenuLabel className="font-normal">
              <div className="flex flex-col space-y-1">
                ...
                <div className="flex flex-col mt-1 space-y-1">
                  <p className="text-xs text-zinc-500 leading-none">{session.user.email}</p>
                  {buildingLocation && (
                    <div className="flex items-center gap-1 text-[10px] text-amber-500 font-medium bg-amber-500/10 px-1.5 py-0.5 rounded w-fit">
                      <MapPin className="w-3 h-3" />
                      <span>{buildingLocation}</span>
                    </div>
                  )}
                </div>
              </div>
            ...
            <Link href={accountLink} className="w-full block">
              <DropdownMenuItem 
